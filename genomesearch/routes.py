...
limiter = Limiter(
    ...
    default_limits=["60000 per day", "5 per second"]
)
...
class GenomeRouting():
    ...
    keysArray = []
    fileNames = [
        'MT wheat Results.txt',
        'US adaptation panel Sthapit SNP results.txt',
        'Utah 90k SNP Results.txt',
        'WA cultivars 90K SNP Results.txt'
    ]

    with open(fileNames[1]) as csvfile:
    ...
    @ app.route('/WRSGGL-Wheat-API-query', methods=['GET'])
    ...
    def all_info():
        ...
        if count == 0:
            outfile = open("./genomeSearch/exit.json", "r")
    ...
    def search():
        ...
        if request.method == 'POST' and form.validate():
            ...
            if messagesCount != None and len(messagesCount) > 0:
                if count == 0:
                    outFile = 'genomesearch/exit.csv'
                else:
            ...
            #    print(increment)
            #    if increment != 'E':
            #        messagesCount = messagesCount.query.filter(increment)
            #        messagesResult = messagesResult.query.filter(increment)
            
            #messagesCount = Genome.query.filter(searchParameters[0]).filter(searchParameters[1]).filter(searchParameters[2]).filter(searchParameters[3]).filter(searchParameters[4]).all()
            #messagesResult = Genome.query.filter(searchParameters[0]).filter(searchParameters[1]).filter(searchParameters[2]).filter(searchParameters[3]).filter(searchParameters[4]).all()


            #if (isFilled(form.lowerCM) and isFilled(form.upperCM)):
            #    messagesCount = Genome.query.filter(Genome.genTrainScore >= form.lowerCM.data).filter(Genome.genTrainScore <= form.upperCM.data).all()
            #    messagesResult = Genome.query.filter(Genome.genTrainScore >= form.lowerCM.data).filter(Genome.genTrainScore <= form.upperCM.data).limit(1000).all()

            #if searchArr != []:
            #    messagesResult = Genome.query.filter(Genome.chrom.in_(searchArr)).all()
            #    messagesCount = Genome.query.filter(Genome.chrom.in_(searchArr)).all()


            #if (form.genomeCultivar.data != ''):          
            #    messagesResult = Genome.query.filter(Genome.samples.contains(form.genomeCultivar.data)).order_by(Genome.chrom).limit(1000).all()
            #    messagesCount = Genome.query.filter(Genome.samples.contains(form.genomeCultivar.data)).all()
            
            
            #print(form.genomeName.data)
            #print(Genome.query.filter_by(SNPId=form.genomeName.data).all())
            
            #if form.genomeChrom.data != None:
            #    messagesResult = Genome.query.filter_by(chrom=form.genomeChrom.data).all()
            #messagesResult = Genome.query.filter(Genome.samples.contains("Admire")).all()
            #searchString = form.genomeCultivar.data
            #print(searchString)
            
            #messagesResult = Genome.query.filter(Genome.samples.contains(searchString)).order_by(Genome.chrom).all()
            #messagesResult = Genome.query.filter(Genome.samples.contains(searchString)).limit(1000).all()
            
            #searchArr = []
            #for chrom in chromArr:
            
            #messagesResult = Genome.query.filter(Genome.chrom.in_(searchArr)).limit(1000).all()
            
            #messagesResult = Genome.query.filter(Genome.genTrainScore.between(form.lowerCM.data, form.upperCM.data)).limit(1000).all()
            #if (isFilled(form.lowerCM) and isFilled(form.lowerCM)):
            #    messagesResult = Genome.query.filter(Genome.genTrainScore >= form.lowerCM.data).filter(Genome.genTrainScore <= form.upperCM.data).limit(1000).all()


            #messagesCount = Genome.query.filter(Genome.samples.contains(searchString)).all()
            #print(Genome.query.filter(Genome.))
            #print(Genome.query.count())
            # None to N/A
            #chromString = messagesResult.chrom
            # (X) TODO: Change adamsBB to just BB 
            # (X) TODO: Change table header to actual name (was previously fixed to Adams)
            # (X) TODO: Database tests for all the chromsome queiries 
            # (X) TODO: Database tests for all the cultivar queiries

            # TODO: Range of chromsome 
            # TODO: Clickable boxes for chromosome? 
            # TODO: Swap out genTrain for CM position (Requires database reset)
            # TODO: Create UI component for searching based CM (46.1 - 54.2)
            # TODO: Filter based upon multiple parameters

            # ISSUE: Searching when A99AR 60092
            
            # TODAY: testing for the database
            
            # Maybe? 2Dx we can delete the x
            # CM position ! (55.2 - 55.4)

            #print(messagesResult, file=sys.stdout)
        
        # TEST DATABASE HERE
        
        allCultivars = "A99AR 600923, Adams, Agent, Alex, Alliance, Andnox, Antelope, Arapahoe, Ark, Arrowsmith, ARS-Selbu, Athena, Auburn, Avoca, Baart 38, Bailey, Barbee, Bergan, Blackhull, Blount's Lambrigg, Brill, Bulter, Capitan, Cataldo, Chiefkan, Cloud, Coker 68-15, Colorow, Copper, Dalen, Delta Queen, Downy, Eagle Chief, Edwall, Eklund, Eltan, Erhardt, Eureka, Extra Early Blackhull, Federation 41, FFR 555W, Fineway, Florida 302, Forward, Franklin, Frontiersman, Fulhio, Funk Seeds 7174, Funk W-504, GA-Gore, GB 2148, Georgia 100, Glyndon, Golden Cross, GR 876, Greer, Hadden, Hansel, Hardired, Hatcher, Hayden, Hi-Line, Hope, Hume, Hymar, IDO 644, III, Indian, InW0731, Iobred, Iowin, James, John, Judith, Kanhull, Karmont, Kenosha, Kitt, Komar, Lamar, LaPorte, Leif, Lemhi 66, Lewis, Lindon, Longhorn, Louise, Louise, Lynn, Madsen, Marett Chancellor, Martin, Maverick, McKay, McNeal, Merit, Michikoff, Mindoro, Missouri Valley, Mosida, Nebraska No. 60, Nelson, New Victory, Newthatch, Nogal, Nordman, Nudel, NY Batavia, Omaha, Onas 53, Oro, Oveson, Paha, Payne, Pecos, Penquito, PL 145, Pomerelle, Pony, Poso 41, Prairie, Probred, Promontory, Protor, Purcell, Racine, Ramona, Rawhide, Red Jacket, Redhard, Redland, Rego, Rew, Ridit, Rio Blanco, Roedel, Rosen, Ruddy, Rushmore, Saline, Satanta, Sawyer, Sentinel, Shepherd, Shortana, Siouxland, Spinkcota, Stacey, Sterling, Sturgeon, Survivor, Syringa, TAM 202, Tammy, Tecumseh, Terral 877, Thorne, Timstein, Tracey, Triplet, Twain, UI Lochsa, Urquie, Ute, Valprize, varname2154, varname2165, varname2510, varname2553, varname2737W, varname5232, varname5469, varname751, varname7833, varname835, Vigal, Voyager, Waco, Wakanz, Wampum 17691, Warrior, Wesel, Westbred Challenger, Wheedling, White Federation 60, Wichita, Willow Creek, Winridge, World Seeds 13, World Seeds 1877, Yamhill, Yorkstar"
        """
        currentCultivar = ""
        for char in allCultivars:
            if char != ',':
                currentCultivar += char
            else:
                currentCultivar.strip()
                print("-------------------")
                print(currentCultivar)
                print("TESTING CULTIVAR NAMES")
                print(currentCultivar + "AA = " + str(Genome.query.filter(Genome.samples.contains(currentCultivar + "AA")).count()))
                print(currentCultivar + "AB = " + str(Genome.query.filter(Genome.samples.contains(currentCultivar + "AB")).count()))
                #print(currentCultivar + "BA = " + str(Genome.query.filter(Genome.samples.contains(currentCultivar + "BA")).count()))
                print(currentCultivar + "BB = " + str(Genome.query.filter(Genome.samples.contains(currentCultivar + "BB")).count()))
                print("TEST CHROMOSOME TYPES " )
                chromArr = ["1A","1B","1D","2A","2B","2Dx","3A","3B","3D","4A","4B","4D", "5A","5B","5D", "6A","6B","6D", "7A","7B","7D"]
                total = 0
                for chrom in chromArr:
                    chromCount = Genome.query.filter_by(chrom=chrom).count()
                    total += chromCount
                    print(currentCultivar + chrom + " = " + str(chromCount))
                #print("TOTAL CHROMOSOME = " + str(total))
                print("-------------------")
                currentCultivar = "" 
        """
        # TODO make sure to return feedback to let the user know their request didn't work       
        print(outFile)
